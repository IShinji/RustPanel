...
### P1-01 系统防火墙与入口安全
- [ ] **P1-01-1** 扩展 `security.proto`：支持 TCP/UDP/ICMP 协议、备注、来源 IP/IP 段限制。
- [ ] **P1-01-2** 后端实现防火墙适配层：支持 UFW/Firewalld/Iptables，增加“禁 Ping”与“防扫描”逻辑开关。
- [ ] **P1-01-3** 开发“安全管理”页面：实现规则增删改查、规则导入导出备份。
- [ ] **P1-01-4** 实现面板“安全入口”自定义：支持自定义访问路径、服务端口，并实现 2FA 两步验证登录。
 
### P1-02 WAF Web 应用防火墙 (Nginx 层)
- [ ] **P1-02-1** 实现抗 CC 攻击逻辑：集成 Nginx 频率限制模块，支持验证码挑战页面分发。
- [ ] **P1-02-2** 实现漏洞防护与关键词拦截：预设 SQL 注入、XSS 攻击过滤规则库。
- [ ] **P1-02-3** 攻击可视化看板：前端开发“全球攻击来源地图”与攻击 IP 排名统计表。
 
### P1-03 SSH 专项加固
- [ ] **P1-03-1** 实现 SSH 端口修改 RPC 及后端配置重载。
- [ ] **P1-03-2** 登录审计与防护：记录登录成功/失败轨迹，支持多次失败尝试后自动封禁攻击源 IP。
- [ ] **P1-03-3** 密钥管理：实现在线生成 RSA/Ed25519 密钥对，一键禁用密码登录。
