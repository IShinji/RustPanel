...
- `P4` PHP/Node/Python **原生**多版本(`@priority: low`):**分场景结论**(详见 P4 段)。能跑 Docker 的
  主机用 AppStore 容器镜像 + 每站点绑定覆盖;**跑不了 Docker 的低配机**(OpenVZ/~128MB)走静态+反代+
  原生进程,不跑动态 PHP(经与用户确认用途为静态/反代/小服务),原生 PHP-FPM 亦不做。
  *(早先"容器化等价覆盖"措辞仅对能跑 Docker 主机成立,已在 P4 段修正。)*
...
## P4 原生多版本运行时 (PHP / Node / Python)  —— 分场景结论

> **结论(本会话定论,经与用户确认)**:按运行形态分层(见 `AGENTS.md` 架构约束)。
> - **能跑 Docker 的主机(KVM + 够内存)**:AppStore 提供 `php:8.x-fpm`、node、python 等
>   容器镜像 + 每站点绑定,覆盖站长需求;原生多版本**不做**(宿主编译/共存多套运行时与
>   "容器优先 + 低配"冲突)。
> - **跑不了 Docker 的低配机(OpenVZ / ~128MB,`can_run_docker=false`)**:定位是静态站(sws)
>   + 反代(rpxy)+ `WorkloadService` 原生进程,**不跑动态 PHP 站**(128MB 跑 php-fpm 本就极限)。
>   经与用户确认该场景用途为"静态/反代/小服务",**原生 PHP-FPM 也不做**;若日后要在 256–512MB
>   低配机跑 PHP,再单独立项补"系统包管理的 php-fpm + 站点绑定 + 池状态"(不依赖 Docker)。
>
> ⚠️ 修正:本条早先写的"容器化等价覆盖站长需求"仅对**能跑 Docker 的主机**成立;低配机走
> 非容器路径,动态 PHP 不在目标内,不是用容器"覆盖"。

- [x] **P4-01~02** ~~PHP/Node/Python 原生多版本~~ —— **决定不做**(高配走容器、低配走静态/反代/原生进程,见上)。 @priority: low
